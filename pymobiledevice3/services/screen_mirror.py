...
class ScreenMirrorService:
    ...
    def __init__(
        ...
        port: int = 8080,
        backend: str = "auto",
        cap: "Optional[ValeriaScreenCapture]" = None,
    ) -> None:
        self._lockdown = lockdown
        self._host = host
        self._port = port
        # Passed through to ValeriaScreenCapture.create() — one of
        # "auto" (default), "cmio" (macOS), "libusb" (Linux/Win).
        self._prefer_backend = backend
        # Optional pre-started capture, owned by the caller. Used by the
        # macOS event-driven entry path where the CLI runs ``cap.run()``
        # on the main thread to drive the CFRunLoop, and the asyncio loop
        # (running on a worker) consumes frames from this pre-started cap.
        # When None, the service creates and owns its own cap.
    ...
    def _quiet_noisy_loggers() -> None:
        ...
        """
        for name in (
            # pymobiledevice3 transport: DTX dumps full payloads at DEBUG.
            "pymobiledevice3.dtx",
    ...
    async def frames(self):
        """Async iterator yielding raw H.264 frame bytes (Annex-B framed).
        ...
        Usage::
            ...
            async with ScreenMirrorService(lockdown) as svc:
                async for h264_bytes in svc.frames():
                    await my_transport.send(h264_bytes)
        """
    ...
    def _broadcast_msg(self, msg: object) -> None:
        ...
        from the live stream (typically <1 s away with the device's encoder)
    ...
    async def _capture_loop(self) -> None:
        """Forward H.264 from the device straight to every WebSocket client.
        ...
        If a pre-started ``cap`` was passed at construction time (the macOS
        event-driven entry path), use it without owning its lifecycle.
        Otherwise create + start + stop our own.
        """
        owns_cap = self._external_cap is None
        if owns_cap:
            udid = getattr(self._lockdown, "identifier", None)
            try:
                cap = ValeriaScreenCapture.create(udid=udid, backend=self._prefer_backend)
            except (BackendUnavailableError, ValueError) as exc:
                raise RuntimeError(f"Cannot start capture: {exc}") from exc
            try:
                # Async-friendly start: yields between runloop ticks so the
                # asyncio loop (and any keep-alives like lockdown's) keeps
                # running during the ~10 s plugin-load wait on macOS.
                await cap.astart()
